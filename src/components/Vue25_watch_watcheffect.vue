<template>
  ...
  <h3>watch는 개발자가 코드로 지정한 변수값의 변화를 감시하는 것, 콜백함수를 이용하여
    부가적인 작업을 추가할 수 있다.
  </h3>
  <p>{{ count_o}}</p>
...
//options API
  ...
  //Composition API
  setup(){
    const count_c_1 = ref(0)//ref를 통해 0으로 초기화
    const count_c_2 = ref(0)
    const state = ref('실행중')
    watch(
      count_c_1, function(cur, prev){
        console.log(`Composition API Watch: ${prev} ==> ${cur}`);
      },{immediate: true,}//immediate: true일 경우 초기값의 변화(undefined-> 0) 파악함.  
    )
    watch(
    ...
    const stop = watchEffect(
      ...
      },{flush: 'post', } //pre : dom이 업데이트 하기전에 콜백함수를 호출
                          //post: dom이 업데이트된 후에 콜백함수를 호출
    )
    const onStop = function(){
      state.value = '중지' //no의미
      stop() //watchEffect를 할당받은 변수 stop을 함수로 호출 stop()하면 watchEffect는 중단
    }
